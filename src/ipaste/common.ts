...
async function handleHistoryClick(ec: any, state: AppState, item: HistoryItem, el: HTMLElement) {
  if (!state.G_Input?.pubkey || !state.G_Input?.salt) {
    setErrMsg(messages.errNeedBookmark);
    return;
  }
...
export async function autoFetchHistory(ec: any, state: AppState) {
  if (!state.G_Input?.pubkey || !state.G_Input?.salt) return;
  try {
