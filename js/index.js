...
!async function() {
    let G_Input, g_urlsafe = 1, ec = await exports.initEC();
    ...
    async function encryptClick() {
        ...
        try {
            ...
            return str = ec.base64Encode(enc, g_urlsafe), document.getElementById("ciphertext").value = str, 
            !0;
        } catch (error) {
    ...
    document.getElementById("encrypt").onclick = async () => {
        ...
        if (base64) try {
            let arr = ec.base64Decode(base64, g_urlsafe), dec = await ec.decrypt(p, arr);
            setPlainText((new TextDecoder).decode(dec));
        } catch (error) {
            setErrMsg(error);
    ...
    async function genbookmark(pubkey, toEmail, prefix, emailSubject) {
        ...
        }, jsonstring = JSON.stringify(s), arr = (new TextEncoder).encode(jsonstring), dataBuff = await ec.encrypt(webPublic, arr), data = ec.base64Encode(dataBuff, 1), bookmark = `${location.origin}${location.pathname}?t=${(new Date).toISOString()}#&data2=${encodeURIComponent(data)}`, a = document.createElement("a");
    ...
    document.getElementById("genbookmark").onclick = async () => {
        ...
        await genbookmark(pubkey, toEmail, prefix, subject);
    }, document.getElementById("build").innerText = "Package:Release  cmt: f9a6fc4 hash: caba47a3 \n 2026-03-12 09:06:39.133 +0800 ", 
    async function() {
        location.hash;
        let search = new URLSearchParams(location.hash), data = search.get("data"), data2 = search.get("data2");
        if (data2 && (data = data2), data && !data2 && (g_urlsafe = 0), !data) return;
        let ttlog = console.log;
        ttlog({
            webPrivate: webPrivate,
            webPublic: webPublic
        });
        let plainBf = await ec.decrypt(webPrivate, ec.base64Decode(data, g_urlsafe)), plain = (new TextDecoder).decode(plainBf);
        ttlog(plain);
